...
module _draw_features(type, tolerance = 0)
{
    translate(v = [ 0, 0, -preview_bump ])
    {
        if (type == 2)
        {
            // intentionally empty to be solid.
        }
        else if (type == 3)
            _insert_countersunk();
        else if (type == 4)
...
module _wall_reliefs(tolerance = 0)
{
    for (i = [0:5])
    {
        rotate([ 0, 0, i * 60 ])
...
module _wall_relief(tolerance = 0)
{
...
module _insert_body(structure, current, tolerance = 0, decorate = true)
{
    // Body.
    chamfer_height = 0.4;
    difference()
...
module _insert_snaps(tolerance = 0, fillet_size = 0)
{
    // Add snaps.
    for (i = [0:5])
    {
...
module _connector(structure, current, quadrant, decorate)
{
    if (_check_location_populated_quadrant(structure, current, quadrant))
    {
        ...
        s6 = ((quadrant == 0 || quadrant == 2) && _check_location_populated_quadrant(structure, current, 1))
                 ? s4 + (1.101 * sin(60))
                 : s4;
